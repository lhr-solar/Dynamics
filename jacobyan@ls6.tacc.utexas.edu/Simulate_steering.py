...
def sim(params):
    ...
    phi_lower_bound = np.radians(15)
    wheel_angle_target = np.deg2rad(20)
    num_fit_points = 100
    ...
    if max(max(np.abs(theta_i)), max(np.abs(theta_o))) < wheel_angle_target:
...
def tierod_angle(a, wt, l1, l2, x):
    ...
    d = np.sqrt(a**2 + (wt-x)**2)#, dtype=np.float64)
